...
def get_dormitory_janitor_count_for_date(db: Session, target_date: datetime.date) -> int:
    """Calculate the number of eligible DORMITORY janitors present for lunch on a given date."""
    total_janitor_ktx = (
        db.query(ForeignEmployee)
        .filter(
            ForeignEmployee.employee_type == "JANITORIAL",
    ...
    return max(0, total_janitor_ktx - absent_ktx_janitors_cnt)
