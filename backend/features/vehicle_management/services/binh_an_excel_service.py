...
def get_bilingual_route(pickup: str, dropoff: str) -> str:
    ...
    if ("kí túc" in p or "ktx" in p) and (
        "công ty" in d or "nienyi" in d or "cn09" in d or "cn15" in d
    ):
        return "kí túc -công ty \n宿舍-公司"
    if (
        "công ty" in p or "nienyi" in p or "cn09" in p or "cn15" in p
    ) and ("kí túc" in d or "ktx" in d):
        return "công ty - kí túc\n公司-宿舍"
    if ("kí túc" in p or "ktx" in p) and (
        "cửa khẩu" in d or "hữu nghị" in d or "友谊关" in d
    ):
        return "kí túc -cửa khẩu \n宿舍- 友谊关"
    if ("cửa khẩu" in p or "hữu nghị" in p or "友谊关" in p) and (
        "kí túc" in d or "ktx" in d
    ):
...
def is_border_trip(d: VehicleDispatch) -> bool:
    text = (
        f"{d.pickup_location or ''} {d.dropoff_location or ''} {d.notes or ''} {d.vehicle_name or ''}"
    ).lower()
    return any(k in text for k in ["cửa khẩu", "hữu nghị", "友谊关", "hnq"])


def build_binh_an_excel_report(
    db: Session, from_date: str, to_date: str
) -> BytesIO:
    wb = Workbook()
    ...
    ws1["A1"] = (
        "CÔNG TY CP THƯƠNG MẠI ĐẦU TƯ VÀ PHÁT TRIỂN DỊCH VỤ DU LỊCH VẬN TẢI BÌNH AN"
    )
    ...
    ws1["A2"] = (
        "ĐC: Ngõ Đền, TDP Hoàng Mai 3, Phường Nếnh, tỉnh Bắc Ninh, Việt Nam  "
    )
    ...
    ws1["A6"] = (
        f"Bảng kê từ ngày {date_range_str} kèm theo hoá đơn số:   kí hiệu:  ngày:"
    )
    ...
    ws1["A7"] = (
        "   Khách hàng客户: CÔNG TY TNHH CÔNG NGHIỆP NIENYI VIỆT NAM                                                                                                                       "
    )
    ...
    for c_idx, h_text in enumerate(headers_1, start=1):
        ...
        cell.font = Font(
            name="Calibri Light", size=11 if c_idx == 6 else 15, bold=True
        )
    ...
    for plate, trips in grouped_trips.items():
        ...
        for d in trips:
            ...
            route_bilingual = get_bilingual_route(
                d.pickup_location or "", d.dropoff_location or ""
            )

            ws1.cell(row=row_idx, column=1, value=stt).alignment = Alignment(
                horizontal="center", vertical="center"
            )
            ws1.cell(row=row_idx, column=2, value=d.dispatch_date).alignment = (
                Alignment(horizontal="center", vertical="center")
            )

            cell_c = ws1.cell(row=row_idx, column=3, value=purpose_bilingual)
            cell_c.alignment = Alignment(
                horizontal="center", vertical="center", wrap_text=True
            )

            cell_d = ws1.cell(row=row_idx, column=4, value=route_bilingual)
            cell_d.alignment = Alignment(
                horizontal="center", vertical="center", wrap_text=True
            )

            ws1.cell(
                row=row_idx, column=5, value=d.vehicle_name or ""
            ...
            ws1.cell(
                row=row_idx, column=8, value=d.license_plate or d.vehicle_name
            ).alignment = Alignment(horizontal="center", vertical="center")
            ...
            cell_i = ws1.cell(row=row_idx, column=9, value=note_str)
            cell_i.alignment = Alignment(
        ...
        # Subtotal row for vehicle plate
        sub_r = row_idx
        ws1.row_dimensions[sub_r].height = 42.0
        ws1.cell(row=sub_r, column=1, value=block_code).alignment = Alignment(
            horizontal="center", vertical="center"
        )
        ws1.cell(row=sub_r, column=2, value=f"TỔNG XE {plate}").alignment = (
            Alignment(horizontal="left", vertical="center")
        )

        cell_sum = ws1.cell(
            row=sub_r, column=7, value=f"=SUM(G{start_row}:G{sub_r - 1})"
        )
    ...
    # Border trips total row (if border trips exist)
    if border_trips:
        block_code = char_codes[char_idx % len(char_codes)]
        char_idx += 1
        ...
        ws1.cell(row=sub_r, column=2, value="TỔNG XE CỬA KHẨU").alignment = (
            Alignment(horizontal="left", vertical="center")
        )
    ...
    ws1.cell(row=grand_r, column=1, value="TỔNG PHẢI TRẢ (VNĐ)").alignment = (
        Alignment(horizontal="left", vertical="center")
    )
    if subtotal_rows:
    ...
    ws2.row_dimensions[1].height = 21.0
    ws2.row_dimensions[2].height = 21.0
    ws2.row_dimensions[3].height = 21.0
    ws2.row_dimensions[4].height = 26.25
    ws2.row_dimensions[5].height = 34.5
    ws2.row_dimensions[6].height = 36.0
    ws2.row_dimensions[7].height = 21.0
    ws2.row_dimensions[8].height = 37.5
    ...
    for d in border_trips:
        ...
        ws2.cell(row=r2_idx, column=1, value=stt2).alignment = Alignment(
            horizontal="center", vertical="center"
        )
        ws2.cell(row=r2_idx, column=2, value=d.dispatch_date).alignment = (
            Alignment(horizontal="center", vertical="center")
        )

        cell_c = ws2.cell(row=r2_idx, column=3, value=purpose_bilingual)
        cell_c.alignment = Alignment(
            horizontal="center", vertical="center", wrap_text=True
        )

        cell_d = ws2.cell(row=r2_idx, column=4, value=route_bilingual)
        cell_d.alignment = Alignment(
            horizontal="center", vertical="center", wrap_text=True
        )

        ws2.cell(
            row=r2_idx, column=5, value=d.vehicle_name or "4座chỗ"
        ).alignment = Alignment(horizontal="center", vertical="center")
        ...
        ws2.cell(
            row=r2_idx, column=7, value=d.license_plate or d.vehicle_name
        ).alignment = Alignment(horizontal="center", vertical="center")

        time_str = f"{d.pickup_time or ''}_{d.return_time or ''}"
        ws2.cell(row=r2_idx, column=8, value=time_str).alignment = Alignment(
            horizontal="center", vertical="center"
        )

        psg_str = f"{d.passenger_count}个人 => {d.passenger_name or ''}"
        ws2.cell(row=r2_idx, column=9, value=psg_str).alignment = Alignment(
    ...
    if border_trips:
        ws2.row_dimensions[r2_idx].height = 47.25
        ws2.cell(row=r2_idx, column=1, value="F").alignment = Alignment(
            horizontal="center", vertical="center"
        )
        ws2.cell(row=r2_idx, column=2, value="TỔNG XE CỬA KHẨU").alignment = (
            Alignment(horizontal="left", vertical="center")
        )

        cell_sum = ws2.cell(
            row=r2_idx, column=6, value=f"=SUM(F9:F{r2_idx - 1})"
        )
        cell_sum.alignment = Alignment(horizontal="right", vertical="center")
        cell_sum.number_format = "#,##0"
